...
contract ParticipantsRoyaltySplitter is PaymentSplitter{
    ...
    constructor (
        address _participantsAddr, address[] memory payees, uint256[] memory shares_
        ) PaymentSplitter(payees, shares_) {

        require(payees.length == shares_.length, "PaymentSplitter: payees and shares length mismatch");
        require(payees.length > 0, "PaymentSplitter: no payees");
        require(_participantsAddr != address(0));
    ...
    function releaseAll() public payable{
        ...
        if ( ethBalance > 0 ){
            for (uint256 payeeIndex = 0; payeeIndex < _payeesCount; payeeIndex++) {
